...
- **DB Driver:** `bun:sql` (VPS) / `@neondatabase/serverless` (CF Workers)
...
│   ├── auth.worker.ts    # CF auth (factory, neon serverless)
...
- CF Workers: auth factory per-request via `auth(c.env)`, `@neondatabase/serverless` driver, `c.env` bindings
